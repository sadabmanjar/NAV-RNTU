...
function MapPage() {
  ...
  const handleLocateMe = () => {
    ...
    watchId.current = navigator.geolocation.watchPosition(
      (position) => {
        ...
        if (isOutsideCampus) {
          showToast("You are outside of the campus please come in the campus then try.");
          if (watchId.current) {
            navigator.geolocation.clearWatch(watchId.current);
      ...
      (error) => {
        ...
        if (error.code === 1) showToast("Location access denied. Please allow location permissions in your browser settings.");
        else if (error.code === 2) showToast("Location unavailable. If you are on mobile, please ensure your GPS is ON and you are using HTTPS or Localhost.");
        else showToast("Failed to get location: " + error.message);
      },
      {
  ...
  const getRouteCoordinates = () => {
    ...
    if (startPoint?.id === 'user-location' && startPoint.lat && startPoint.lng) {
              ...
              <select
                ...
                onChange={(e) => {
                  if (e.target.value === 'user-location') {
                    ...
                    if (!userLocation) {
                        showToast("Fetching your location... Please wait and ensure GPS is enabled.");
                        handleLocateMe();
                    }
                  } else {
